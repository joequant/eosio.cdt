	...
	if [ -z "${DNF}" ]; then
		printf "\\n\\tDnf must be installed to compile EOS.IO.\\n"
		printf "\\n\\tExiting now.\\n"
		exit 1;
	fi
	...
	then
		printf "\\n\\tDNF update failed with the above errors.\\n"
		printf "\\n\\tExiting now.\\n"
		exit 1;
	fi
	...
	printf "\\n\\tChecking DNF for installed dependencies.\\n\\n"

	for (( i=0; i<${#DEP_ARRAY[@]}; i++ ));
	do
		pkg=$( sudo "${DNF}" info "${DEP_ARRAY[$i]}" 2>/dev/null | grep Repo | tr -s ' ' | cut -d: -f2 | sed 's/ //g' )
	...
	if [ ${COUNT} -gt 1 ]; then
		...
		select yn in "Yes" "No"; do
			case $yn in
				[Yy]* ) 
					...
					then
						printf "\\n\\tDNF dependency installation failed.\\n"
						printf "\\n\\tExiting now.\\n"
						exit 1;
					else
						printf "\\n\\tDNF dependencies installed successfully.\\n"
					fi
				break;;
		...
		printf "\\n\\tNo required DNF dependencies to install.\\n"
	fi
